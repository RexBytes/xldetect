...
Two numbers decide whether xldetect is shippable:
...
through review (most defects found this way are docstring/code contract gaps,
not logic errors); and behaviour depends on empirical facts about dependencies
(which exact exception a stdlib or `openpyxl` call raises). The review cycle is
the spec-discovery mechanism. So the goal is not "zero cycles" — it is a
...
| Security posture | 5 | malformed-file / exception-normalisation tests present |
